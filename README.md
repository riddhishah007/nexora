...
> **Deployment status:** `Phase 34 Path A ($0 Free Permanent)` merged `core-api+worker` (`services/core-api/app/main.py:34` lifespan) → single `Free` web service `render.yaml:1` (`plan: free`, `dockerContext: .`, `healthCheckPath: /health`) + `Supabase` pgvector (`supabase/enable_pgvector.sql:3`) + `Upstash` Redis + `Vercel Hobby` (`apps/web`). Local still `docker compose` (`docker-compose.yml:40` `core-api` build `context: .` + vendored `worker` at `services/core-api/app/worker.py:1`). Next: create `Supabase`/`Upstash` projects, set `DATABASE_URL`/`REDIS_URL` secrets on `Render`, deploy `Vercel` with `NEXT_PUBLIC_API_URL`. Verify: `python scripts/smoke.py --base https://<render>.onrender.com` → 7 `PASS`, `curl /metrics` → text/plain.
