...
> **Deployment status:** ✅ **LIVE — Phase 35/36 Everything (2026-09-01).** Backend `https://nexora-core-api.onrender.com` (`render.yaml:1` `plan: free` single `nexora-core-api` + merged `core-api+worker` `services/core-api/app/main.py:34` + Supabase `supabase/enable_pgvector.sql:3` + Upstash) and frontend `https://web-nine-snowy-57.vercel.app/` are deployed and verified: `python scripts/smoke.py --base https://nexora-core-api.onrender.com` → **7/7 PASS** (`GET /health` + `X-Request-ID` `app/middleware/request_id.py:1`, `GET /metrics` `app/routers/metrics.py:26`, `GET /` + `GET /agents` 7 agents + `POST /agents/run` 401 + `POST /rag/search` 401 + `register→login→GET /agents` + offline RAG eval) + `python scripts/demo_portfolio.py` → **DEMO PASS** (approvals → notifications → exports). `app/worker.py:12` `JOB_DEAD` dead-letter fix + `render.yaml:15` `autoDeploy:false` → manual deploy `941b5f1`. Vercel `apps/web` builds **21 routes** (`/projects` `app/models/project.py:6` + `/approvals` `app/models/approval.py:14` + `/notifications` `app/models/notification.py:8` + `/showcase` `apps/web/src/app/showcase/page.tsx:14` + `/history` `/knowledge` + etc. via `apps/web/src/components/app-shell.tsx:12` 14-link nav, fixed `rootDirectory: apps/web` `prj_a8MOBfUkygaTczv5NsVJ93vsyvE7`). **Keep-alive:** `scripts/keepalive.py:1` + `.github/workflows/keepalive.yml:1` pings `/health` every 10 min (Free sleeps 15 min `docs/DEPLOYMENT.md:45`); **Demo video:** `docs/DEMO_VIDEO_SCRIPT.md:1` 2-min Loom script → record and paste Loom link here + in Showcase hero. Local still `docker compose` (`docker-compose.yml:40` `context: .` + `alembic 0010` live via `Dockerfile CMD alembic upgrade head`).

**Phase 35/36 — Everything (2026-09-01).** Option A `alembic 0010` approvals `app/routers/approvals.py:14` (`POST /approvals/{id}/decision` `§14`) + notifications `app/routers/notifications.py:14` + exports `app/routers/exports.py:14` + Option B showcase `/showcase` + 10 scenarios `docs/DEMO_SCENARIOS.md:1` + `scripts/demo_portfolio.py:1` + Vercel rootDirectory fix + keepalive. `next build` 11.7s → 21/21 `○`. Push → Vercel auto-deploys `NEXT_PUBLIC_API_URL=https://nexora-core-api.onrender.com/api/v1` (`render.yaml:35` CORS allowlist).
