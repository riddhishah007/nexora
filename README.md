...
> **Deployment status:** not deployed yet — everything currently runs locally via `docker compose` (`http://localhost:3000` web, `http://localhost:8000/docs` API). Deployment configs are prepped for later: `docs/DEPLOYMENT.md`, `render.yaml`, `fly.worker.toml`. Next up after build-out: usage/metrics dashboard (`api_usage` recorded at ~20 call sites, no aggregation endpoint/UI yet), test suite + evals (zero tests today), token-by-token answer streaming.
