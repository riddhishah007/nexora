...
## Status
🚧 **Phase 25 — Token-by-token answer streaming.** `GroqProvider.generate_stream` (SSE, `stream: true`) with a stream-safe `<think>` filter (`filter_think_chunks`: stateful across chunk boundaries — split opener/closer tags handled, Qwen reasoning dropped, answer passed through; unit-tested). `LLMGateway.generate_stream` falls back to single-chunk for non-streaming providers. New `synthesize_final_answer_streaming` emits `SYNTHESIS_DELTA`/`SYNTHESIS_DONE` over the existing workflow WS channel during multi-step chat runs (falls back to blocking generate if the stream breaks; same 429 backoff policy as non-streaming). Chat pending bubble renders the synthesis live with a blinking caret as deltas arrive. Also fixed latent bug: `emit` was never imported in `chat.py`, so failure-path notifications silently NameError'd since Phase 22. Verified E2E: search→writer plan produced **218 SYNTHESIS_DELTA events**; streamed text matches the persisted assistant answer; 45/45 tests pass.

**Phase 24 — Test suite.** First tests in the repo: 45 passing (`pytest` + `pytest-asyncio`). Unit: injection scanner, planner `_parse` + fallbacks via fake gateway, Groq `<think>` strip + streaming filter, filename sanitization. Integration (auto-skip when stack down): health, auth roundtrip, registry shape, templates, workflow CRUD/validation, usage summary scoping, auth boundaries. Run: `docker exec -w /app nexora-core-api python -m pytest tests/ -v`.
...
> **Deployment status:** not deployed yet — everything currently runs locally via `docker compose` (`http://localhost:3000` web, `http://localhost:8000/docs` API). Deployment configs are prepped for later: `docs/DEPLOYMENT.md`, `render.yaml`, `fly.worker.toml`. Next up after build-out: RAG golden-set evals (needs real embeddings), real cost estimation (`estimated_cost` is hardcoded to 0), streaming synthesis for the workflow builder too.
