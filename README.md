...
> **Deployment status:** not deployed yet — everything currently runs locally via `docker compose` (`http://localhost:3000` web, `http://localhost:8000/docs` API). Deployment configs are prepped for later: `docs/DEPLOYMENT.md`, `render.yaml`, `fly.worker.toml`. CI: `.github/workflows/ci.yml` (backend+frontend+docker) + smoke `scripts/smoke.py --base http://localhost:8000` (7 checks). Live: `GET /metrics` → Prometheus counters, `GET /health` → `X-Request-ID` + JSON logs. Next up: live deploy.
